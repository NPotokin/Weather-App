...
const AdvancedWindForecastGraph = () => {
  ...
  const searchForecast = (event) => {
    axios.get(unitsC ? urlCForecast : urlFForecast).then((response) => {
      ...
      const dataWind = response.data.list.map((item) => item.wind.speed);
      const dataTime = response.data.list.map((item) => item.dt_txt);
      const graphData = dataTime.map((item, index) => {
        return { time: item, value: dataWind[index] };
      });
      setDataWind(dataWind);
...
export default AdvancedWindForecastGraph;
