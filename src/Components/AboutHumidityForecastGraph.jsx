...
const AboutHumidityForecastGraph = () => {
  ...
  const searchForecast = (event) => {
    axios.get(unitsC ? urlCForecast : urlFForecast).then((response) => {
      ...
      const dataHumidity = response.data.list.map((item) => item.main.humidity);
      const dataTime = response.data.list.map((item) => item.dt_txt);
      const graphData = dataTime.map((item, index) => {
        return { time: item, value: dataHumidity[index] };
      });
      setDataHumidity(dataHumidity);
...
export default AboutHumidityForecastGraph;
