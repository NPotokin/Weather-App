...
const AdvancedCloudsForecastGraph = () => {
  ...
  const searchForecast = (event) => {
    axios.get(unitsC ? urlCForecast : urlFForecast).then((response) => {
      ...
      const dataClouds = response.data.list.map((item) => item.clouds.all);
      const dataTime = response.data.list.map((item) => item.dt_txt);
      const graphData = dataTime.map((item, index) => {
        return { time: item, value: dataClouds[index] };
      });
      setDataClouds(dataClouds);
...
export default AdvancedCloudsForecastGraph;
